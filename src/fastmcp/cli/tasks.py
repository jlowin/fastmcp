...
def check_docket_enabled() -> None:
    ...
    if not fastmcp.settings.experimental.enable_docket:
        console.print(
            ...
            "  [cyan]export FASTMCP_EXPERIMENTAL_ENABLE_DOCKET=true[/cyan]\n\n"
            "Then try again."
        )
        sys.exit(1)

    docket_url = fastmcp.settings.experimental.docket.url
    ...
    if docket_url.startswith("memory://"):
        console.print(
            ...
            "   export FASTMCP_EXPERIMENTAL_DOCKET_URL=redis://localhost:6379/0\n\n"
...
def worker(
    ...
    async def run_worker():
        ...
        async with server._lifespan_manager():
            ...
            console.print(f"  Backend: {fastmcp.settings.experimental.docket.url}")
            console.print(
                f"  Concurrency: {fastmcp.settings.experimental.docket.concurrency}"
            )
