...
A comprehensive MCP server with 21 tools across 10 categories:
- System: OS info, environment variables
- Files: Directory listing, file reading
- Utility: Safe commands, calculations
- Git: Repository status, diffs
- Search: File and content search
- JSON: Parse and format JSON
- HTTP: Web requests and URL checking
- Text: Statistics, regex, hashing
- Network: Port checking, DNS lookup
- DateTime: Current time, timestamp parsing

Usage:
    Local (STDIO):  python -m activ8_mcp.server
    Web (HTTP):     python -m activ8_mcp.server --http --port 8000
"""
...
mcp = FastMCP(
    "Activ8MCP",
    instructions="Activ8-AI custom MCP server with system, file, git, and utility tools.",
)
...
def http_get(url: str, timeout: int = 10) -> dict[str, Any]:
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "Activ8MCP/1.0"})
...
def check_url(url: str, timeout: int = 5) -> dict[str, Any]:
    ...
    try:
        req = urllib.request.Request(url, method="HEAD", headers={"User-Agent": "Activ8MCP/1.0"})
        with urllib.request.urlopen(req, timeout=timeout) as response:
            return {
                "reachable": True,
                "status": response.status,
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Activ8-AI MCP Server")
