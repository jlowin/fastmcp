...
The final SEP-1686 specification has **MAJOR breaking changes** from our prototype implementation. The most critical change is that **clients no longer provide task IDs** - servers must generate them. Additionally, field names have changed (`keepAlive` → `ttl`, `pollFrequency` → `pollInterval`), new required fields were added (`createdAt`), the initial task status changed (`submitted` → `working`), and the capabilities structure was refined.
...
DOCKET_TO_MCP_STATE: dict[ExecutionState, str] = {
    ExecutionState.SCHEDULED: "submitted",  # ← Should be "working"
    ExecutionState.QUEUED: "submitted",      # ← Should be "working"
    ExecutionState.RUNNING: "working",       # ✓ Correct
    ExecutionState.COMPLETED: "completed",   # ✓ Correct
    ExecutionState.FAILED: "failed",         # ✓ Correct
    ExecutionState.CANCELLED: "cancelled",   # ✓ Correct
}
```
...
**Our Implementation:**
- ✓ We respect TTL via Docket's `execution_ttl`
- ✗ We don't track `createdAt`
- ✗ We return hardcoded values: `keepAlive: 60000`, `pollFrequency: 1000` (`protocol.py:115-116`)
- ✗ We don't override client-requested TTL (we don't even receive it correctly!)
...
**Our Implementation:**
- ✓ We include `modelcontextprotocol.io/related-task` in result responses
- ✗ We include it in `tasks/get` responses (spec says SHOULD NOT - lines 447-448)
...
**Differences:**
1. Field names wrong (`keepAlive` → `ttl`, `pollFrequency` → `pollInterval`)
...
**Protocol Tests:**
- `test_task_protocol.py` - Generic protocol tests ✓
- `test_task_capabilities.py` - Capability declaration ✗ (will fail due to structure changes)
- `test_task_metadata.py` - Metadata handling ✗ (field names changed)
- `test_task_methods.py` - Operation methods ✓

**Component Tests:**
- `test_task_tools.py` - Tool task execution ✗ (status/field names)
- `test_task_prompts.py` - Prompt task execution ✗
- `test_task_resources.py` - Resource task execution ✗
- `test_task_dependencies.py` - DI in task context ✓
- `test_task_return_types.py` - Return value conversion ✓

**Client Tests:**
- `test_client_tasks.py` - Client task operations ✗ (field names)
- `test_client_tool_tasks.py` - Client tool tasks ✗
- `test_client_prompt_tasks.py` - Client prompt tasks ✗
- `test_client_resource_tasks.py` - Client resource tasks ✗
- `test_task_context_validation.py` - Context validation ✓
- `test_task_result_caching.py` - Result caching ✓
- `test_client_task_protocol.py` - Protocol operations ✗

**Other:**
- `test_task_keepalive.py` - TTL handling ✗ (renamed to ttl)
- `test_task_security.py` - Security/isolation ✓
- `test_server_tasks_parameter.py` - task= parameter ✗
- `test_sync_function_task_disabled.py` - Sync function handling ✓
...
2. **Field name changes**
   - Rename all `keepAlive` → `ttl`
   - Rename all `pollFrequency` → `pollInterval`
...
4. **Change initial status**
   - Change "submitted" → "working" for new tasks
...
| Field Names | keepAlive→ttl, pollFrequency→pollInterval | protocol.py, handlers.py, tasks.py, converters.py, tests | YES | P0 |
| createdAt | Add required timestamp field | handlers.py, protocol.py | YES | P0 |
| Initial Status | Change "submitted"→"working" | handlers.py, protocol.py | YES | P0 |
