...
**Publish manual docs updates through a PR.** The `published-docs` branch serves gofastmcp.com, and repository rules reject direct pushes and force-pushes to it. For prereleases and later docs follow-ups, create a temporary branch from the current `published-docs`, make a single commit whose tree exactly matches the desired commit on `main`, and open a PR with `published-docs` as the base. Merging that PR publishes to production. Never push directly to `published-docs`.

**Merge the docs changelog PR *before* cutting the release, not after.** The post-publish `update-published-docs` job updates `published-docs` to the released commit for stable releases on the default branch, so the changelog entry only reaches the live site if it's already in the commit being tagged. Land the docs PR on the release target branch first, then cut the release from that branch. If you tag first and merge docs after, this release's changelog won't appear on the live site until the next default-branch stable release updates `published-docs`, or until a maintainer publishes `main` manually through the PR flow above. Maintenance releases from `release/3.x` or `release/2.x` publish packages and GitHub notes without repointing `published-docs`; add their changelog entries on the maintenance branch, slotted into the matching major-version section. Two hand-maintained files mirror the GitHub release and must get a new entry for every version, newest at the top (these are `.mdx` and are not covered by the prek Prettier hook, which only runs on `yaml`/`json5` — match the existing entries' style by hand):
