...
from fastmcp.server.middleware.mcp_middleware import MiddlewareContext
...
class TestMethods:
    ...
    async def test_list_tools(self, client, recording_middleware: RecordingMiddleware):
        async with client:
        ...
        calls = recording_middleware.get_calls(hook="on_list_tools_request")
        assert len(calls) == 1
        assert calls[-1].context.source == "client"
    ...
    async def test_progress(
        ...
        calls = recording_middleware.get_calls(hook="on_progress_notification")
        assert len(calls) == 1

        # server-initiated notifications should have source "server"
        assert calls[0].context.source == "server"
    ...
    async def test_logging(self, mcp_server, recording_middleware: RecordingMiddleware):
        ...
        calls = recording_middleware.get_calls(hook="on_logging_message_notification")
        assert len(calls) == 1

        # server-initiated notifications should have source "server"
        assert calls[0].context.source == "server"

    async def test_sample(self, mcp_server, recording_middleware: RecordingMiddleware):
        ...
        async with Client(mcp_server, sampling_handler=sampling_handler) as client:
            await client.call_tool("sample_tool", {})

        recording_middleware.assert_called("on_message", "sampling/createMessage")
        recording_middleware.assert_called("on_request", "sampling/createMessage")
        recording_middleware.assert_called(
            "on_create_message_request", "sampling/createMessage"
        )

        calls = recording_middleware.get_calls(hook="on_create_message_request")
        assert len(calls) == 1
        assert calls[-1].context.source == "server"
        assert calls[0].payload.params.messages[0].content.text == "hello"  # type: ignore[attr-defined]
        assert calls[0].result.content.text == "This is the sample message!"  # type: ignore[attr-defined]

    async def test_ping(self, client, recording_middleware: RecordingMiddleware):
        async with client:
            await client.ping()

        recording_middleware.assert_called("on_message", "ping")
        recording_middleware.assert_called("on_request", "ping")
        recording_middleware.assert_called("on_ping_request", "ping")


class TestMiddleware:
    async def test_recording_middleware_doesnt_fire_for_unused_methods(
        self, client, recording_middleware: RecordingMiddleware
    ):
        async with client:
            await client.list_tools()

        recording_middleware.assert_called("on_list_tools_request", "tools/list")
        recording_middleware.assert_not_called(
            "on_list_resources_request", "tools/list"
        )
    ...
    async def test_nested_middleware_with_server_wrapped_notifications(
        self, mcp_server: FastMCP, recording_middleware: RecordingMiddleware
    ):
        """
        Server-initiated notifications should be wrapped in a ServerNotification
        RootModel and call_next hooks are dynamically re-wrapped. We introduce a
        chain of middleware to ensure this wrapping is taking place. If this
        test fails, it probably means that the unwrapped notification is being
        passed instead of the wrapped one.
        """
        mcp_server.add_middleware(RecordingMiddleware())
        test_middleware = RecordingMiddleware()
        mcp_server.add_middleware(test_middleware)
        mcp_server.add_middleware(RecordingMiddleware())
        ...
        test_middleware.assert_called("on_progress_notification", times=1)
        calls = test_middleware.get_calls(hook="on_progress_notification")
        assert len(calls) == 1
        assert calls[0].context.source == "server"
