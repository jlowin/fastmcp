...
class TestCustomToolNames:
    ...
    async def test_add_tool_object_with_custom_name_via_model_copy(self):
        """Test adding a Tool object with a different name using model_copy."""
        ...
        manager = ToolManager()
        # Use model_copy to create a new tool with a new name
        tool_with_new_name = tool.model_copy(update={"name": "proxy_tool"})
        manager.add_tool(tool_with_new_name)
        # The tool is accessible under the new name
        stored = await manager.get_tool("proxy_tool")
        assert stored is not None
        assert stored.name == "proxy_tool"
