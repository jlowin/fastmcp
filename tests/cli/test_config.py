...
class TestFileSystemSource:
    ...
    def test_resolve_entrypoint_does_not_execute_source(self, tmp_path: Path):
        ...
        entrypoint = source.resolve_entrypoint()

        assert entrypoint == "app"
        assert not side_effect.exists()

    async def test_resolve_entrypoint_skips_non_server_common_binding(
        self, tmp_path: Path
    ):
        server_file = tmp_path / "server.py"
        server_file.write_text(
            "from fastmcp import FastMCP\n"
            'mcp = FastMCP("Overwritten")\n'
            'mcp = "metadata"\n'
            'server = FastMCP("Selected")\n'
        )
        source = FileSystemSource(path=str(server_file))

        entrypoint = source.resolve_entrypoint()
        loaded_server = await source.load_server()

        assert entrypoint == "server"
        assert loaded_server.name == "Selected"

    def test_resolve_entrypoint_requires_explicit_unverified_constructor(
        self, tmp_path: Path
    ):
        server_file = tmp_path / "server.py"
        server_file.write_text(
            "from custom import FastMCP\n"
            "from fastmcp import FastMCP as VerifiedServer\n"
            "mcp = FastMCP()\n"
            'server = VerifiedServer("Selected")\n'
        )
        source = FileSystemSource(path=str(server_file))

        with pytest.raises(ValueError, match="provide an explicit entrypoint"):
            source.resolve_entrypoint()

    def test_resolve_entrypoint_accepts_public_mcp_server_import(self, tmp_path: Path):
        server_file = tmp_path / "server.py"
        server_file.write_text(
            'from mcp.server import MCPServer\nmcp = MCPServer("Selected")\n'
        )
        source = FileSystemSource(path=str(server_file))

        assert source.resolve_entrypoint() == "mcp"

    def test_resolve_entrypoint_accepts_typed_server_constructor(self, tmp_path: Path):
        server_file = tmp_path / "server.py"
        server_file.write_text(
            "from fastmcp import FastMCP\n"
            "class State: pass\n"
            'mcp = FastMCP[State]("Selected")\n'
        )
        source = FileSystemSource(path=str(server_file))

        assert source.resolve_entrypoint() == "mcp"

    def test_resolve_entrypoint_accepts_proxy_factory(self, tmp_path: Path):
        server_file = tmp_path / "server.py"
        server_file.write_text(
            "from fastmcp.server import create_proxy\n"
            'mcp = create_proxy("backend.py")\n'
        )
        source = FileSystemSource(path=str(server_file))

        assert source.resolve_entrypoint() == "mcp"

    def test_resolve_entrypoint_rejects_proxy_from_wrong_module(self, tmp_path: Path):
        server_file = tmp_path / "server.py"
        server_file.write_text(
            "from fastmcp import FastMCP, create_proxy\n"
            'mcp = create_proxy("backend.py")\n'
            'app = FastMCP("Selected")\n'
        )
        source = FileSystemSource(path=str(server_file))
    ...
    def test_resolve_entrypoint_rejects_non_server_classmethod(self, tmp_path: Path):
        server_file = tmp_path / "server.py"
        server_file.write_text(
            "from fastmcp import FastMCP\n"
            "server = FastMCP.generate_name()\n"
            'app = FastMCP("Selected")\n'
        )
        source = FileSystemSource(path=str(server_file))

        assert source.resolve_entrypoint() == "app"

    def test_resolve_entrypoint_accepts_server_classmethod(self, tmp_path: Path):
        server_file = tmp_path / "server.py"
        server_file.write_text(
            "from fastmcp import FastMCP as Server\n"
            "app = Server.from_openapi(openapi_spec={})\n"
        )
        source = FileSystemSource(path=str(server_file))

        assert source.resolve_entrypoint() == "app"

    def test_resolve_explicit_entrypoint_without_parsing_source(self, tmp_path: Path):
        server_file = tmp_path / "server.py"
        server_file.write_text("not valid Python {{")
        source = FileSystemSource(path=f"{server_file}:create_server")

        assert source.resolve_entrypoint() == "create_server"

    def test_resolve_entrypoint_requires_explicit_name_for_dynamic_common_binding(
        self, tmp_path: Path
    ):
        server_file = tmp_path / "server.py"
        server_file.write_text(
    ...
    def test_resolve_entrypoint_requires_explicit_name_after_uncertain_write(
        self, tmp_path: Path
    ):
        server_file = tmp_path / "server.py"
        server_file.write_text(
    ...
    def test_resolve_entrypoint_rejects_stale_match_capture(self, tmp_path: Path):
        server_file = tmp_path / "server.py"
        server_file.write_text(
            "from fastmcp import FastMCP\n"
            "mcp = FastMCP('Stale')\n"
    ...
    def test_resolve_entrypoint_requires_supported_binding(self, tmp_path: Path):
        server_file = tmp_path / "server.py"
        server_file.write_text("mcp = build_server()\n")
        source = FileSystemSource(path=str(server_file))
