---
title: regex
sidebarTitle: regex
---

# `fastmcp.server.transforms.search.regex`


Regex-based search transform.

## Classes

### `RegexSearchTransform` <sup><a href="https://github.com/jlowin/fastmcp/blob/main/src/fastmcp/server/transforms/search/regex.py#L16" target="_blank"><Icon icon="github" style="width: 14px; height: 14px;" /></a></sup>


Search transform using regex pattern matching.

Tools are matched against their name, description, and parameter
information using ``re.search`` with ``re.IGNORECASE``.

