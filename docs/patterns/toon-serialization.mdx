...
**TOON's sweet spot:** Uniform arrays of objects – multiple fields per row, same structure across items. It combines YAML's indentation-based structure with CSV's tabular format, optimized for token efficiency.
...
**Note:** The Python TOON implementation is currently in beta. If unavailable, FastMCP gracefully falls back to JSON.
...
If TOON format is requested but the `toon_format` package is not installed:
...
TOON is **ideal** for:
- ✅ **Uniform arrays of objects** with consistent structure
- ✅ Multiple fields per row, same schema across items
- ✅ Tabular data (like CSV but with nested support)
- ✅ LLM input where token efficiency matters
- ✅ Reducing costs in high-volume LLM applications

TOON may be **less efficient** for:
- ❌ Deeply nested structures with varying shapes
- ❌ Non-uniform data (different fields per object)
- ❌ Sparse data with many optional fields
- ❌ LLM output generation (TOON is designed for input)
...
**Current Status:** 
- ✅ Infrastructure and API ready
- ⚠️ Requires Python TOON implementation (reference is TypeScript/JS)
- ✅ Falls back to JSON if TOON unavailable
- ✅ Full test coverage for JSON path
