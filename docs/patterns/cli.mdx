...
The `stdio` subcommand outputs the shell command that an MCP host would use to start your server over stdio transport. This is useful when you can never quite remember the `uv run --with fastmcp fastmcp run ...` incantation, or when you need to paste the command into a tool or script that doesn't have a dedicated install target.
...
When given a `fastmcp.json`, dependencies from the configuration are automatically included:
