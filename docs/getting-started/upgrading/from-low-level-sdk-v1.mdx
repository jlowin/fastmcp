...
MCP SDK v2 landed sweeping breaking changes on the low-level `Server`, and every one of them lands on code like yours. The protocol types moved out of `mcp.types` into a separate `mcp_types` package. Every field was renamed from camelCase to snake_case. `McpError` was renamed. And the `Server` class itself was rebuilt: handlers are no longer registered with decorators but passed to the constructor as `on_*` callables, and their signatures changed from `(name, arguments)` to `(ctx, params)`, returning wrapped result objects instead of bare lists. Staying on the low-level SDK means rewriting your imports, every handler signature, every handler return, and your error construction.

**Migrating to FastMCP is generally less work than migrating to raw SDK v2** — and it is less work by a wide margin, because the low-level handler machinery is exactly the code that FastMCP deletes. You are not porting those handlers to a new signature; you are throwing them away. FastMCP 4 runs on SDK v2 and hides that whole surface behind a high-level API: you write `@mcp.tool`, FastMCP derives the protocol layer from your function signature, and the rename simply isn't something your code has to know about. You come out the other side with the framework as well — composition, middleware, proxies, authentication, and in-process testing. The SDK v2 break is the natural moment to make the jump.
...
- `lifespan=` and anything reading the lifespan context
...
- direct session use for anything else — flag it, since FastMCP does not hand you the raw session
