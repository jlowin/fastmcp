...
```
ToolError: elicitation via server-initiated requests is unavailable on 2026-07-28 connections; return an InputRequiredResult instead (SEP-2322)
```
...
**`DeclinedElicitation` and `CancelledElicitation` are now falsey.** In 3.x every elicitation result was truthy, so `if result:` treated a refusal as approval. In 4.0 a declined or cancelled result is false; an accepted result stays truthy even when `data` is `False` or `"no"`. `if result:` is a correct approval guard; it is still not a check of the answer value. See [Elicitation](/servers/elicitation#requesting-input-on-handshake-connections).
