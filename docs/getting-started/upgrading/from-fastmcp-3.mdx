...
9. **Verify behavior changes.** Confirm templated resources that legitimately accept `..` or absolute paths are exempted, update any client that matched the old `-32002` resource-not-found code, and if your OAuth server sets `issuer_url` to a value other than `base_url`, schedule the [one-time re-authorization](#behavior-changes-to-verify) its clients now need.
