...
`name`, `version`, `instructions`, `icons`, `website_url`, `lifespan`, `tools`, `resource_security`, and `request_state_security` all mean what they meant before. The rest map like this:
