...
- `cache_hints=`
- `extensions=`
These raise TypeError, most naming their replacement. `name`, `version`, `instructions`, `icons`, `website_url`, `lifespan`, `tools`, `resource_security`, and `request_state_security` carry over unchanged.

CONTEXT
- `ctx.mcp_server` → `ctx.fastmcp`
- `ctx.headers` → `get_http_headers()` from `fastmcp.server.dependencies` (a function, not a Context property — the attribute does not exist on FastMCP's Context and would raise AttributeError)
- `ctx.notify_tools_changed()` and its siblings — send the notification directly with `ctx.send_notification(ToolListChangedNotification())` from `mcp_types`. FastMCP emits it for you when components change visibility through its own `ctx.enable_components` / `ctx.disable_components`.
- everything else on Context (logging, progress, elicit, read_resource, request_id, input_responses, request_state) carries over unchanged
...
The `tools=` row is easy to miss, because the keyword survives while its contents do not. `MCPServer(tools=[...])` takes the SDK's own `Tool` objects; `FastMCP(tools=[...])` takes plain callables or FastMCP's `Tool`, and raises `TypeError` on an SDK tool. Pass the underlying functions instead — FastMCP rebuilds the schema from each signature, which is the same derivation `Tool.from_function` was doing for you:
...
The two are era-gated in both directions: `ctx.elicit()` raises on a modern connection, and a guard result raises on a handshake one. A server that must serve both branches on `ctx.protocol_version`. See [Elicitation](/servers/elicitation#which-approach-to-use) for both shapes side by side.
