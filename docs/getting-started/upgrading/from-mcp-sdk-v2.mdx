...
- `cache_hints=`
- `extensions=`
- `tools=[...]` — the keyword survives but its contents do not: FastMCP takes plain callables (or its own `Tool`) and raises TypeError on an SDK `Tool`. Pass the underlying functions.
