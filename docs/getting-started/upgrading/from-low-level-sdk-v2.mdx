...
- `ctx.params`, `ctx.request_id`, `ctx.meta`, `ctx.protocol_version` — on a FastMCP Context these live on `ctx.request_context` (note `ctx.protocol_version` itself does not exist)
