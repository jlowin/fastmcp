...
- `lifespan=` and anything reading `ctx.lifespan_context`

HANDLERS TO DELETE (each becomes one or more decorated functions)
- `on_list_tools` + `on_call_tool` — note the `if params.name == ...` dispatch chain inside `on_call_tool`; each branch becomes its own tool
- `on_list_resources` + `on_list_resource_templates` + `on_read_resource`
- `on_list_prompts` + `on_get_prompt`
- `on_completion`
- `on_subscribe_resource` / `on_unsubscribe_resource` / `on_subscriptions_listen`
- `on_set_logging_level`, `on_progress`, `on_roots_list_changed`, `on_ping`
...
- `ctx.params`, `ctx.request_id`, `ctx.meta`, `ctx.protocol_version`
- Direct `ctx.session` use for anything else — flag it, since FastMCP does not hand you the raw session
...
### Converting schemas to type hints
...
One thing to check as you migrate: FastMCP does not hand you the raw `ServerSession`. If a handler reaches into `ctx.session` for something with no `Context` equivalent, that code needs a rethink rather than a translation.
