...
FastMCP has pioneered MCP development since FastMCP 1.0 was incorporated into the official MCP SDK in 2024. Today, 70% of MCP servers across all languages run on some version of FastMCP, because **FastMCP automatically gives you the best practices for working with the Model Context Protocol.**

FastMCP 3.0 is built for the next generation of MCP applications:
...
The architecture that enables this—[components](/servers/tools), [providers](/servers/providers/overview), and [transforms](/servers/transforms/transforms)—is covered throughout the docs.
