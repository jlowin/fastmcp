...
This is extremely useful for testing your FastMCP servers. FastMCPTransport can simulate both `stdio` and `streamable-http` transports, making it a versatile tool for testing different transport scenarios without needing separate server processes.
...
client = Client(server)  # Transport is automatically inferred as stdio mode
...
#### Advanced Usage (Streamable HTTP)

<VersionBadge version="2.6.0" />

You can also simulate a streamable-http connection for testing HTTP-specific behavior:
...
server = FastMCP(name="HttpTestServer")

@server.tool()
def get_data():
    return {"message": "Hello from HTTP simulation"}

# Create a client with streamable-http transport simulation
transport = FastMCPTransport(server, transport="streamable-http")
client = Client(transport)

async def main():
    async with client:
        result = await client.call_tool("get_data")
        print(f"HTTP simulation result: {result}")
...
<Warning>
When using `transport="streamable-http"`, SSE-based streaming features are **not supported** due to restrictions of the in-memory ASGI transport. This includes:

- Progress reporting (`context.report_progress()`)
- Sampling callbacks
- Other real-time streaming features

The streamable-http mode is primarily intended for testing HTTP transport logic and request/response patterns, not for testing streaming capabilities.
</Warning>

Communication happens through efficient in-memory queues (in stdio mode) or simulated HTTP requests (in streamable-http mode), making both very fast and ideal for unit testing.
