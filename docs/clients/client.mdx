...
<Note>
A multi-server configuration presents one aggregate MCP endpoint, so every backend shares one negotiated protocol era and one set of connection settings. To hold several independent connections — each with its own protocol `mode`, authentication, and handlers — see [Client Groups](/clients/client-groups) (experimental).
</Note>
