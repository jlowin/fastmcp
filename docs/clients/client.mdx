...
The `initialize()` method is idempotent - calling it multiple times returns the cached result from the first successful call. You can also use it to access initialization details with a shorter syntax:

```python
async with Client(mcp) as client:
    # Access initialization result directly
