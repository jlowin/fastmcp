...
fastmcp run fastmcp.json --no-env
```

### Environment Setup Control

By default, FastMCP uses `uv` to create an isolated environment based on your configuration. The `--no-env` flag allows you to skip this automatic environment setup:
