...
fastmcp run fastmcp.json --skip-env-setup
```

### Environment Setup Control

By default, FastMCP uses `uv` to create an isolated environment based on your configuration. The `--skip-env-setup` flag allows you to skip this automatic environment setup:

```bash
fastmcp run fastmcp.json --skip-env-setup
```

**When to use `--skip-env-setup`:**
