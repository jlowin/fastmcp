...
<Warning>
You **must** ensure that your `.well-known` routes are accessible at the root level of your domain. FastMCP can not do this automatically if your server is mounted under a path prefix. Follow the instructions below to configure your server correctly.
</Warning>
...
Two parameters control where these routes are located:
...
**`base_url`** tells clients where to find operational endpoints. This includes any mount path prefix:

```python
base_url="http://localhost:8000/api"
```

These URLs differ when mounting under a prefix because discovery metadata lives at the root while operational endpoints live under the mount path.
