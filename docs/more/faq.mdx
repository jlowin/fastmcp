...
You do not need to pin it just because you registered a `sampling_handler`, `roots=`, or an `elicitation_handler`. [Roots](/clients/roots) and [elicitation](/clients/elicitation) both have a modern-era form — the guard pattern delivers them through `InputRequiredResult`, and the same handler you already registered answers it, no extra wiring required. [Sampling](/clients/sampling) has no FastMCP 4 server API on either era (`ctx.sample()` is gone entirely), so a `sampling_handler` only ever answers a server that is not running FastMCP 4 and still pushes `sampling/createMessage` over the handshake — and `mode="auto"` already falls back to the handshake against that kind of server on its own.
