...
Beyond the core components, MCP supports richer interaction patterns. A server can send asynchronous **notifications** to a client, and a tool can pause to ask the user a question partway through its work.

How a tool asks depends on the protocol era. The modern protocol (`2026-07-28`) is sessionless, so a server cannot interrupt a connected client; instead the tool returns a description of what it needs and the client calls back with the answer. That change also removed server-initiated **sampling**, where a server borrowed the client's LLM mid-call. On FastMCP 4, generate text by [calling an LLM from your server](/servers/sampling) instead.
