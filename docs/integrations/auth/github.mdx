...
GitHub uses OAuth scopes to control what your application can access. The `GitHubProvider` defaults to the `user` scope for basic profile access.
...
The `GitHubProvider` handles several GitHub-specific requirements:
...
This complexity is hidden behind the simple `GitHubProvider` interface, giving you GitHub authentication with minimal configuration.
