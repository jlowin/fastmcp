...
Issuer URL for OAuth metadata (defaults to `BASE_URL`). Set to root-level URL when mounting under a path prefix to avoid 404 logs. See [ASGI Integration](/integrations/asgi#mounting-oauth-protected-servers) for details.
</ParamField>
