...
We recommend including `offline_access` in `additional_authorize_scopes` to obtain refresh tokens for long-lived server sessions. FastMCP manages token refreshing automatically.
