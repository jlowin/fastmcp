...
|------------|---------|----------|
| Custom API scopes | `read`, `write` | Prefixed with `identifier_uri` (e.g., `api://xxx/read`) |
| OIDC scopes | `openid`, `profile`, `email`, `offline_access` | Sent as-is, excluded from token validation |
| Microsoft Graph | `User.Read`, `Mail.Send` | Sent as-is (dot-notation recognized) |
| Fully-qualified | `api://xxx/read` | Sent as-is (already prefixed) |
