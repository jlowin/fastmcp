...
For development, you can use tools like `ngrok` or `inspectr` to temporarily expose a locally-running server to the internet. We'll do that for this example by using `inspectr` since it comes with a UI for [MCP insights](https://inspectr.dev/docs/examples/expose-your-fastmcp/#mcp-insights) (or you may need to install `ngrok` which requires to create an account), but you can use any other method to deploy your server.
...
<Warning>
This exposes your unauthenticated server to the internet, with a public URL like `https://my-fastmcp.in-spectr.dev`. Only run this command in a safe environment if you understand the risks.
</Warning>
...
url = 'https://my-fastmcp.in-spectr.dev'
