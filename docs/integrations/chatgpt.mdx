...
Your server must be accessible from the internet. For development, you can use `inspectr` or `ngrok`, or any other method to deploy your server.
For this example, we'll use [inspectr](https://inspectr.dev/docs/examples/expose-your-fastmcp/#mcp-insights), since it runs with just one command to tunnel your local FastMCP server and it comes with a handy UI for MCP insight & tracing options.
...
```bash Terminal 2
npx @inspectr/inspectr --backend=http://localhost:8000 --expose
```
</CodeGroup>

From the terminal output, you can find your public URL (e.g., `https://my-fastmcp.in-spectr.dev`) for the next steps.
More information about the `inspectr` CLI tool and it's configuration options can be found in the [Inspectr documentation](https://inspectr.dev/docs/configuration/cli-options/).
...
2. Enter:
   ...
   - **Server URL**: `https://my-fastmcp.in-spectr.dev/mcp/`
