...
Your server must be accessible from the internet. For development, you can use `inspectr` or `ngrok`:
For this example, we'll use [inspectr](https://inspectr.dev/docs/examples/expose-your-fastmcp/#mcp-insights), since it runs with just one command and comes with a handy UI for MCP insight & Tracing options.
