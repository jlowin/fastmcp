...
auth_provider = AWSCognitoProvider(
    ...
    domain_prefix="my-app",                  # Your AWS Cognito domain prefix
...
async def get_user_info() -> dict:
    """Returns information about the authenticated AWS Cognito user."""
    from fastmcp.server.dependencies import get_access_token

    token = get_access_token()
    # The AWSCognitoProvider extracts user data from JWT claims
    return {
...
async def main():
    ...
    async with Client("http://localhost:8000/mcp/", auth="oauth") as client:
        ...
        print(f"AWS Cognito user: {result['username']}")
...
<ParamField path="FASTMCP_SERVER_AUTH_AWS_COGNITO_DOMAIN_PREFIX" required>
Your AWS Cognito domain prefix (e.g., `my-app` for `my-app.auth.region.amazoncognito.com`)
</ParamField>
...
<ParamField path="FASTMCP_SERVER_AUTH_AWS_COGNITO_TIMEOUT_SECONDS" default="10">
HTTP request timeout for AWS Cognito API calls
</ParamField>
</Card>
...
async def get_user_profile() -> dict:
    ...
    return {
        ...
        "groups": token.claims.get("cognito_groups", [])
    }
```
...
async def admin_only_tool() -> str:
    ...
    token = get_access_token()
    user_groups = token.claims.get("cognito_groups", [])
