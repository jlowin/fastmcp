...
Transforms modify components (tools, resources, prompts) as they flow through the system. Available transforms:

- `Namespace` - adds prefixes to names and path segments to URIs
- `ToolTransform` - modifies tool schemas (rename, description, tags)
- `Visibility` - filters components (internal, backs enable/disable API)
...
# "foo" → "api_foo" (namespace) → could rename further with ToolTransform
```
