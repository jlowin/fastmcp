...
Because FastMCP's FastAPI integration is based on its [OpenAPI integration](/zh/integrations/openapi), you can customize how endpoints are converted to MCP components in exactly the same way. For example, here we use a `RouteMap` to map all GET requests to MCP resources, and all POST/PUT/DELETE requests to MCP tools:
...
<Tip>
To learn more about customizing the conversion process, see the [OpenAPI Integration guide](/zh/integrations/openapi).
</Tip>
...
For more details on configuration options, see the [OpenAPI Integration guide](/zh/integrations/openapi).