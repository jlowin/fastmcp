...
The one rule that comes with this: an app name must be unique within a server. Two copies of `FastMCPApp("notes")` under different namespaces are indistinguishable to the resolver, and FastMCP will refuse to bind rather than pick one. Name them for what they serve — `FastMCPApp("notes-acme")` and `FastMCPApp("notes-globex")`.
