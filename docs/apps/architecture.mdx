...
A reference this server cannot resolve is left alone rather than corrupted, so a server that does not recognize a tool degrades to the identity-addressed form instead of emitting a broken name.
