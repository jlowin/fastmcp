...
Backend tools appear in `tools/list` like any other tool — the host is what keeps them away from the model. What they can't rely on is a stable *name*: namespace transforms rename them, so `save_contact` becomes `contacts_save_contact` in one composition and something else in another, while the renderer was built before any of that was decided.

Hashed lookup solves that. FastMCP first tries normal tool resolution. If no tool matches and the requested name looks like `<hash>_<local_name>`, FastMCP calls `get_tool_by_hash(hash, local_name)`. That lookup walks the provider tree directly, skipping transforms. It finds an app-visible tool by its original registered name and verifies that its stored `meta["fastmcp"]["tool_hash"]` matches the requested hash.

The hash is public metadata, deterministic from the app name and the tool's registered name. Because it survives every transform and every wire boundary, an intermediary holding a renamed tool can still tell which tool it is.
