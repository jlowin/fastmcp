...
An app name must be unique within a server. The identity is derived from the app name and the tool's registered name, so composing the same app twice produces two tools claiming one identity — and nothing in the listing says which copy a given UI belongs to.

FastMCP refuses that rather than guessing. Neither copy's name is bound, the reference keeps its identity-addressed form, and calling it reports the ambiguity:
...
Give each copy its own app name. Two tenants running the same product need `FastMCPApp("contacts-acme")` and `FastMCPApp("contacts-globex")`, not two instances of `FastMCPApp("contacts")` under different namespaces — namespaces rename tools, and the identity is deliberately immune to renaming.
