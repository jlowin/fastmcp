...
-   **`ResourceResult`**: Wrap multiple contents with result-level metadata. See [ResourceResult](#resourceresult) below.
...
When you need to return multiple pieces of content or add metadata at the result level, use `ResourceResult`. This wrapper type follows the same pattern as `ToolResult` and `PromptResult` elsewhere in FastMCP.
...
def get_items() -> ResourceResult:
    """Returns multiple items with result-level metadata."""
...
`ResourceResult` accepts two fields:

**`contents`** - The resource content to return. This can be a single value (str, bytes, dict, etc.) or a list of values. Raw values are automatically normalized to `ResourceContent` objects, so you can pass simple types directly.

**`meta`** - Optional result-level metadata dictionary included in the MCP response's `_meta` field. Use this for metadata about the overall read operation rather than individual content items.

The automatic normalization means you can pass raw values without wrapping them in `ResourceContent`:

```python
# These are equivalent:
return ResourceResult("hello")
return ResourceResult(ResourceContent(content="hello", mime_type="text/plain"))

# Pass multiple items as a list
return ResourceResult(["item1", "item2"])

# Mix raw values with ResourceContent for fine-grained control
return ResourceResult([
    "plain text item",
    ResourceContent(content="<html>...</html>", mime_type="text/html"),
])
```

<Note>
`ResourceResult.meta` and `ResourceContent.meta` serve different purposes. Use `ResourceContent.meta` for metadata specific to a single piece of content (like CSP headers for an HTML widget). Use `ResourceResult.meta` for metadata about the overall read operation (like total counts or version information).
</Note>
