...
Additionally, you can manage resources states via API endpoints:

```bash
# These endpoints are secured if authentication is enabled
POST /resources/{uri:path}/enable
POST /resources/{uri:path}/disable
# They also apply to templates
...
curl -X POST \
  ...
  http://localhost:8001/resources/URI:PATH/enable
```
