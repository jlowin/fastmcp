...
The `_meta` field is stripped from arguments before they reach your function, so your component implementation never sees it. This convention allows version selection to work across any MCP client without requiring protocol changes.

For resources, the same mechanism works through the MCP request's `_meta` field (which is part of the protocol's request params, not arguments). The FastMCP client handles this automatically when you pass the `version` parameter.
