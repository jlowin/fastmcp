...
You can configure how the FastMCP server handles attempts to register multiple prompts with the same name. The `on_duplicate` setting covers every component type, so it applies to prompts alongside tools and resources.
...
mcp = FastMCP(
    name="PromptServer",
    on_duplicate="error"  # Raise an error if a name is duplicated
)
