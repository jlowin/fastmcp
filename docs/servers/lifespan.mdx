...
Access the lifespan context in tools via `ctx.request_context.lifespan_context`:
...
Existing `@asynccontextmanager` lifespans still work:
...
To compose with other lifespans, use `|` - the `@asynccontextmanager` function will be wrapped automatically:
...
# Can compose @asynccontextmanager functions with @lifespan functions
combined = legacy_lifespan | new_lifespan
```
