...
`DeclinedElicitation` and `CancelledElicitation` are falsey; `AcceptedElicitation` is always truthy, including when `data` is a negative answer (`False`, `"no"`). That makes `if result:` a correct *approval* guard — a refusal no longer looks like consent — but it is not a check of the answer itself:

```python
@mcp.tool
async def create_candidate(ctx: Context) -> str:
    result = await ctx.elicit("Create the candidate?", response_type=bool)
    if result:            # accepted; declined and cancelled are falsey
        if result.data:   # the user answered yes
            return "created"
    return "not created"
```

`if result:` alone is the wrong check when the accepted answer *is* the decision. An accepted `"no"` is still an `AcceptedElicitation`. `result.accepted` is the same boolean as `bool(result)` if you want the intent written out.

The same trap exists on the [modern-protocol path](#elicitation-on-the-modern-protocol): an `ElicitResult` in `ctx.input_responses` is a result object, not the answer. Check `action == "accept"` *and* the value before treating it as approval.
...
Always check `answer.action` before reading `answer.content`: a client may **decline** or **cancel**, in which case `content` is absent. A decline is a normal answer, not an error — it is delivered to your tool like any other round so you can handle it deliberately. The result object itself is always truthy, so `if answer:` is not an approval guard the way `if result:` is for `ctx.elicit()`. Check the action *and* the value.
