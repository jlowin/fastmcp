...
<Warning>
`require_roles` cannot signal a scope shortfall. OAuth has no way to request a role, so there is no `insufficient_scope` challenge to emit and the check is opaque. Because a single opaque check withholds scope disclosure for every check beside it, combining `require_roles` and `require_scopes` on one component stops that component from emitting the step-up challenges described in [Signaling Scope Shortfalls](#signaling-scope-shortfalls). Apply them at different layers — for example scopes on the component and roles in middleware — if you need both.
</Warning>
