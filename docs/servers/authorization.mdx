...
That rule also bounds what gets aggregated. A scope requirement paired with a custom check is left out of the combined shortfall, because whether that layer would admit the caller is unknown until it runs, and running it early would trigger authorization logic the request had not reached yet. The reported set is therefore complete for scope-only requirements and conservative elsewhere: it may name fewer scopes than the full chain requires, but it never names scopes behind a policy that might reject the caller regardless.
