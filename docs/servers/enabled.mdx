...
<Note>
Server-level visibility changes affect all connected clients. For per-session filtering where different clients see different components, see [Session Visibility](/servers/context#session-visibility).
</Note>
