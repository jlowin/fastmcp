...
<Tip>
**Background Tasks**: For sampling in background tasks (`task=True`), use `TaskContext` instead of `Context`. See [Elicitation and Sampling in Background Tasks](/servers/tasks#elicitation-and-sampling-in-background-tasks).
</Tip>
