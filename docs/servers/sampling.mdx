...
<ResponseField name="ctx.sample" type="async method">
  ...
  <Expandable title="Parameters">
    ...
    <ResponseField name="tool_choice" type="ToolChoice | None" default="None">
...
result = await ctx.sample(messages=prompt, tools=tools, tool_choice=ToolChoice(mode="auto"))

# Force the LLM to use a tool
result = await ctx.sample(messages=prompt, tools=tools, tool_choice=ToolChoice(mode="required"))

# Prevent tool use (useful for final response)
result = await ctx.sample(messages=prompt, tools=tools, tool_choice=ToolChoice(mode="none"))
```
...
<Note>
Sampling with tools requires either a client that advertises the `sampling.tools` capability, or a fallback handler configured with `sampling_handler_behavior="always"`. Most current MCP clients don't yet advertise tool support, so using a fallback handler is the recommended approach for tool-enabled sampling.
</Note>
