...
For requests that need conversational context, construct a list of `SamplingMessage` objects representing the conversation history. Each message has a `role` ("user" or "assistant") and `content`.
...
async def contextual_analysis(query: str, data: str, ctx: Context) -> str:
    ...
    messages = [
        SamplingMessage(role="user", content=f"Here's my data: {data}"),
        SamplingMessage(
            role="assistant",
            content="I see the data. What would you like to know?"
        ),
        SamplingMessage(role="user", content=query)
    ]
...
When you need validated, typed data instead of free-form text, use the `result_type` parameter with a Pydantic model. FastMCP ensures the LLM returns data matching your schema, handling validation and retries automatically.
...
Combine tools with `result_type` for agentic workflows that return validated, structured data. The LLM uses your tools to gather information, then returns a response matching your Pydantic schema.
...
- `step.is_text` — True if the LLM returned a text response
