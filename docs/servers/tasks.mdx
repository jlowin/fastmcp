...
## Elicitation and Sampling in Background Tasks

<VersionBadge version="2.16.0" />

Background tasks can request user input (elicitation) or LLM responses (sampling) using the `TaskContext` dependency. This implements the MCP `input_required` status flow per SEP-1686.

<Warning>
The regular `Context` dependency is **not available** in background tasks because workers may run in separate processes without an active MCP session. Use `TaskContext` instead.
</Warning>

### TaskContext Dependency

Import `CurrentTaskContext()` to access elicitation and sampling from within background tasks:
...
@mcp.tool(task=True)
async def clarify_and_execute(
    query: str,
    task_ctx: TaskContext = CurrentTaskContext()
) -> str:
    """A long-running task that needs user clarification."""
    
    # Ask the user for clarification
    result = await task_ctx.elicit(
        message=f"Your query '{query}' is ambiguous. Please clarify:",
        response_type=UserChoice,
    )
    
    if result.action == "accept":
        return f"Executing with option: {result.data.option}"
    return "User declined to clarify"
```

When `elicit()` or `sample()` is called, `TaskContext` automatically:

1. Updates task status to `input_required`
2. Sends the request with `related-task` metadata  
3. Waits for the response
4. Restores task status to `working`

Clients see the task status change and can display appropriate UI for user input.

### Elicitation in Tasks

The `TaskContext.elicit()` method mirrors `Context.elicit()` from foreground tools:
...
async def destructive_operation(
    ...
    match result:
        case AcceptedElicitation(data=conf) if conf.proceed:
            # ... perform deletion ...
...
Use `TaskContext.sample()` to request LLM completions from within background tasks:
...
async def research_and_summarize(
    ...
    """Research a topic and get an AI summary."""
    
    # ... gather research data ...
    research_data = f"Found information about {topic}..."
    
    # Request LLM to summarize
...
<Note>
`TaskContext.sample()` returns the raw `CreateMessageResult` from the client, not the enriched `SamplingResult` that `Context.sample()` provides. For simple text extraction, use `result.content.text`.
</Note>

### Limitations

`TaskContext` is only available in **embedded worker mode** (the FastMCP default). Distributed workers running in separate processes cannot access the session and will raise an error.

For distributed workers, consider:
- Pre-collecting all needed information before starting the background task
- Using a message queue pattern for async communication
- Keeping interactive tasks in the embedded worker
