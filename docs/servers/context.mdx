...
class UserAuthMiddleware(Middleware):
    async def on_call_tool(self, context: MiddlewareContext, call_next):
        ...
        if context.fastmcp_context:
            context.fastmcp_context.set_state_value("user_id", "user_123")
            context.fastmcp_context.set_state_value("permissions", ["read", "write"])
...
async def secure_operation(data: str, ctx: Context) -> str:
    ...
    user_id = ctx.get_state_value("user_id")  # "user_123"
    permissions = ctx.get_state_value("permissions")  # ["read", "write"]
