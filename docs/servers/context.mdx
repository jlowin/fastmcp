...
- **STDIO Transport**: For STDIO connections, the session is equivalent to the lifetime of the server process. The session ID will be generated once and persist for the entire server lifetime.
- **Session ID Availability**: The `session_id` is available in all contexts, including `on_initialize` middleware, allowing you to track sessions from the moment a client connects.
