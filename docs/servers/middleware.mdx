...
<Tip>
Use the FastMCP Context object to store state data that your tools can access later. For example, middleware can authenticate a user and store their permissions in `context.fastmcp_context.set_state_value("permissions", ["read", "write"])`, then tools can access this with `ctx.get_state_value("permissions")`. See [Context State Management](/servers/context#state-management) for details.
</Tip>
