...
<Accordion title="Tool hangs or doesn't respond on cancellation">
  ...
  **Cause:** The decorator explicitly re-raises `asyncio.CancelledError` to allow proper cancellation propagation.
    ...
    **Solution:** Ensure your async code uses `await` properly and doesn't suppress `CancelledError`. The decorator handles this automatically—verify your underlying function respects cancellation tokens.
</Accordion>
