...
proxy = create_proxy(ProxyClient("backend_server.py"))
...
### Adding Proxy Provider to Existing Server

Use `create_proxy_provider()` to add proxied components to an existing server:
...
# Add proxied tools from remote server
