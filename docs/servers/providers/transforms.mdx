...
FastMCP provides two transforms for common scenarios.
...
The `ToolTransform` transform modifies tool schemas. It renames tools, changes descriptions, modifies tags, and adjusts argument schemas.
...
`ToolTransform` is useful when you want to modify tools from mounted or proxied servers without changing the original source. Provide a dictionary mapping original tool names to their transformation configuration.
...
FastMCP provides two mechanisms for modifying tools. Both use the same configuration options; the difference is timing.
...
**Deferred transformation** with `ToolTransform` applies modifications when tools flow through a transform chain. Use this for tools from providers you don't control directly.
...
Use `ToolTransform` for tools from mounted servers, proxies, or other providers where you don't have direct access to the tool objects.
...
external_server = FastMCP("External")

@external_server.tool
def api_get_user_data(uid: str, include_metadata: bool = False) -> dict:
    """Retrieve user data from the API."""
    return {"user_id": uid, "metadata": include_metadata}

# Mount and transform
main = FastMCP("Main")
mount = main.mount(external_server, namespace="ext")
mount.add_transform(ToolTransform({
    "ext_api_get_user_data": ToolTransformConfig(
        name="get_user",
        description="Get information about a user.",
        arguments={
            "uid": ArgTransformConfig(
                name="user_id",
                description="The user's unique identifier.",
            ),
            "include_metadata": ArgTransformConfig(hide=True, default=False),
        },
    ),
}))

# Clients see "get_user" with a cleaner interface
```
