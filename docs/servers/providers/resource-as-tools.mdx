---
title: Resource as Tools
sidebarTitle: Resource as Tools
...
Some MCP clients only support tools. They cannot list or read resources directly because they lack resource protocol support. The `ResourceAsTools` transform bridges this gap by generating tools that provide access to your server's resources.

When you add `ResourceAsTools` to a server, it creates two tools that clients can call instead of using the resource protocol:
...
Pass your server to `ResourceAsTools` when adding the transform. The transform queries that server for resources whenever the generated tools are called.
