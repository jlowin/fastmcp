...
This pattern exists primarily to support the MCP protocol specification's requirements. **Most users should strongly prefer the [Remote Authentication pattern](/servers/auth/remote-authorization)** to integrate with a dedicated identity provider like WorkOS, Auth0, or Okta.
</Warning>
