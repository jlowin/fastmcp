...
`RemoteAuthProvider` combines token validation with OAuth discovery metadata, enabling MCP clients to automatically discover and authenticate with external identity providers that support Dynamic Client Registration (DCR).
...
For example, the built-in `AuthKitProvider` is a `RemoteAuthProvider` that uses WorkOS AuthKit as the external identity provider:
...
`RemoteAuthProvider` excels for production applications that need professional identity management without implementation complexity, when your identity provider supports DCR.

→ **Complete guide**: [Remote OAuth](/servers/auth/remote-oauth)

### OAuthProxy

`OAuthProxy` enables authentication with OAuth providers that don't support Dynamic Client Registration (DCR), such as GitHub, Google, Azure, and most enterprise identity systems.

This class acts as a bridge between MCP's DCR expectations and traditional OAuth providers. It implements DCR locally by always returning your pre-registered credentials, while transparently forwarding OAuth flows to your upstream provider.

For example, the built-in `GitHubOAuthProxyProvider` is a `OAuthProxy` that uses GitHub as the external identity provider:
...
`OAuthProxy` is essential when you need to integrate with established OAuth providers that require manual app registration.
...
**For integrating with existing OAuth providers like GitHub, Google, or Azure, use OAuth Proxy.** This approach enables authentication with providers that don't support Dynamic Client Registration, which includes most established OAuth systems. OAuth Proxy handles the complexity of bridging MCP's DCR requirements with traditional OAuth flows, making it the right choice when you already have OAuth apps registered with these providers.

**For new applications that need professional identity management, choose providers that support DCR.** Services like WorkOS AuthKit are designed for modern authentication flows and provide enterprise-grade features without implementation complexity. These providers offer the best balance of security, features, and simplicity when you're starting fresh.
