...
CIMD support is enabled by default. You can customize its behavior with these parameters:
...
<ParamField body="cimd_trust_policy" type="CIMDTrustPolicy | None">
  Controls how CIMD clients are treated during authorization. You can configure trusted domains that skip consent entirely, or block specific domains from connecting.
...
auth = OAuthProxy(
    ...
    cimd_trust_policy=CIMDTrustPolicy(
        # These domains skip the consent screen entirely
        trusted_domains=["vscode.dev", "internal.mycompany.com"],
        # These domains are always rejected
        blocked_domains=["malicious-site.com"],
    ),
)
```

Subdomain matching is enabled, so trusting `example.com` also trusts `sub.example.com`.
...
By default, all CIMD clients show a consent screen with their verified domain. You can configure trust policies to automatically approve clients from domains you control or commonly use:
...
auth = OAuthProxy(
    ...
    token_verifier=token_verifier,
    base_url="https://your-server.com",
    # Trust VS Code and internal tools to skip consent
    cimd_trust_policy=CIMDTrustPolicy(
        trusted_domains=["vscode.dev", "insiders.vscode.dev"],
    ),
)
...
Trusted domains bypass the consent screen entirely, providing a seamless experience for known clients while maintaining security for unknown ones.
