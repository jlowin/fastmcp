...
These are the three redirect forms RFC 8252 sanctions for native apps, and native clients accept nothing else. In particular, non-loopback plain `http` is rejected, as are standard IANA-registered schemes such as `ftp:`, `ws:`, `smb:`, `smtp:`, and `nfs:` — a private-use scheme is by definition an *unregistered* one based on a domain the app controls. Both application types always reject unsafe browser schemes (`javascript:`, `data:`, `file:`, `vbscript:`).
