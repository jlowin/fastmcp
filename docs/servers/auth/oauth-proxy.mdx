...
| ------------------ | --------------------- |
| `"native"` (default) | `https` URLs; loopback `http` (`http://127.0.0.1`, any address in `127.0.0.0/8`, `http://localhost`, subdomains such as `http://app.localhost`, `http://[::1]`, any port); and private-use schemes (e.g. `com.example.app:/callback`, `myapp://callback`) |
| `"web"` | `https` on a non-loopback host only |

These are the three redirect forms RFC 8252 sanctions for native apps, and native clients accept nothing else. Non-loopback plain `http` is rejected, as is any IANA-registered scheme — `ftp:`, `ws:`, `smb:`, `smtp:`, `nfs:`, `coap:`, `stun:`, `mqtt:`, and the rest. RFC 8252 §7.1 defines a private-use scheme as an *unregistered* one based on a domain the app controls, so FastMCP tests the scheme against a vendored snapshot of the IANA URI scheme registry rather than a hand-maintained list. A scheme nobody anticipated therefore fails closed. Both application types always reject unsafe browser schemes (`javascript:`, `data:`, `file:`, `vbscript:`).
