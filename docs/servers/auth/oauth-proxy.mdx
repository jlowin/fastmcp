...
| ------------------ | --------------------- |
| `"native"` (default) | `https` URLs; loopback `http` (`http://127.0.0.1`, any address in `127.0.0.0/8`, `http://localhost`, `http://[::1]`, any port); and private-use schemes (e.g. `com.example.app:/callback`, `myapp://callback`) |
