...
OAuth Proxy uses ephemeral (in-memory) storage by default, which causes authentication errors when the server restarts. This section explains the common errors you'll encounter and how to fix them.

### Common Error Messages

<AccordionGroup>

<Accordion title='"invalid_token" - Authentication failed (HTTP 401)' icon="triangle-exclamation">

**Where:** Connecting to the MCP endpoint (`/mcp`)

**Error message:**
```json
{
  "error": "invalid_token",
  "error_description": "Authentication failed. The provided bearer token is invalid, expired, or no longer recognized by the server..."
}
```

**What it means:** Your client's access token is no longer valid.

**Common causes:**
- Server restarted with ephemeral storage (tokens lost)
- Token expired (typically after 1 hour)
- Token was revoked

**How to fix:**
- **Immediate**: Clear auth tokens in your client and reconnect
- **Long-term**: Configure [persistent storage](/servers/storage-backends) (see below)

</Accordion>

<Accordion title='"Client Not Registered" - HTML page in browser (HTTP 400)' icon="browser">

**Where:** Browser window during authorization

**What you see:** Styled error page saying "The client ID `xxx` is not registered with this server."

**What it means:** Your client opened the browser to authorize, but it's not registered with the server.

**Common causes:**
- Server restarted and lost client registrations
- First time connecting
- Client registration expired

**What happens:**
1. Your client (VS Code, Claude Desktop, etc.) opens your browser
2. Browser gets HTTP 400 and displays this page
3. Your client doesn't see the error - it's waiting for callback that never arrives

**How to fix:**
1. Close the browser window
2. Clear auth tokens in your client (or restart it)
3. Reconnect - your client will automatically re-register

**Why can't the server redirect back with an error?**

OAuth 2.0 security rules forbid redirecting when `client_id` is unknown - we can't verify the `redirect_uri` is legitimate.

</Accordion>

<Accordion title='"invalid_request" - Client ID not found JSON (HTTP 400)' icon="code">

**Where:** API/programmatic clients calling `/authorize`

**Error message:**
```json
{
  "error": "invalid_request",
  "error_description": "Client ID 'xxx' is not registered...",
  "registration_endpoint": "https://your-server.com/register",
  "authorization_server_metadata": "https://your-server.com/.well-known/oauth-authorization-server"
}
```

**What it means:** Same as "Client Not Registered" above, but JSON response instead of HTML.

**Who sees this:**
- Programmatic/API clients (not browser-delegated)
- Browser-delegated clients see HTML instead

**For programmatic clients:**
The `registration_endpoint` field enables automatic recovery - POST to register, then retry.

</Accordion>

<Accordion title='"invalid_client" - Client authentication failed (HTTP 401)' icon="key">

**Where:** Token exchange at `/token`
...
**What it means:** The `client_id` and `client_secret` don't match a registered client.

**Common causes:**
- Server restarted (lost registrations)
- Wrong credentials
- Client was deleted/revoked

**How to fix:** Clear tokens and reconnect - your client will re-register.
...
By default, OAuth Proxy stores everything in memory:
- Client registrations
- JWT signing keys
- Token encryption keys

When the server restarts, everything is lost.
...
auth = OAuthProxy(
    # ... upstream OAuth config ...

    # Persistent client registrations
    client_storage=SqliteStore("oauth_clients.db"),

    # Persistent keys (CRITICAL!)
...
<Warning>
**Persistent keys are critical:** Without explicit keys, FastMCP generates random keys at startup. Random keys → tokens become invalid on restart, even with persistent client storage.

Generate keys once and store in environment variables:
```bash
...
**Storage options:**
- **SQLite**: File-based, good for single-server deployments
- **Redis**: Distributed, excellent for multi-server
- **PostgreSQL**: Enterprise-grade

See [Storage Backends](/servers/storage-backends) for complete documentation.

### Why Can't Clients Auto-Detect These Errors?

**Browser-delegated OAuth (VS Code, Claude Desktop, etc.):**

These clients **cannot** detect authorization failures automatically:

1. Client opens your browser to `/authorize`
2. Browser sees the error
3. **Client is blind** - waiting for callback that never arrives

This is an OAuth limitation, not a bug.

**Why no error callback?**

OAuth 2.0 spec forbids redirecting when `client_id` is unknown - can't verify `redirect_uri` is legitimate (prevents phishing).

**Programmatic clients:**

Direct API clients can detect errors (they receive JSON with `registration_endpoint`) and automatically recover.

**Best solution:** Configure persistent storage to prevent the problem.
