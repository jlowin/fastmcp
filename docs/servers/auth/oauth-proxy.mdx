...
<VersionBadge version="2.13.0" />
  Secret used to sign FastMCP JWT tokens issued to clients. Accepts any string or bytes, which is stretched into a 32-byte cryptographic key using PBKDF2 (1.2M iterations), since a supplied secret may be low-entropy.

  **Default behavior (`None`):**
  Derives a 32-byte key from the upstream client secret using HKDF.
