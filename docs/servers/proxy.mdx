...
<Tip>
FastMCP proxies automatically handle session management to ensure safe concurrent operations. When multiple clients make simultaneous requests through the same proxy, each request gets its own isolated session to prevent context mixing.
</Tip>
...
`FastMCP.as_proxy()` automatically creates fresh sessions for each request to ensure safe concurrent operations. You can simply pass a `Client` instance and the proxy will handle session management appropriately:

```python
from fastmcp import FastMCP, Client

# Simple approach - let as_proxy() handle everything
client = Client("backend_server.py")
proxy = FastMCP.as_proxy(client)
```

The proxy automatically detects whether the client is connected and creates the appropriate session strategy to prevent context mixing during concurrent requests.
...
# Standard approach - pass a client
client = Client("backend_server.py")
proxy = FastMCPProxy(client=client)

# Advanced approach - provide a client factory for custom control
def create_client():
...
- **`client`**: A `Client` instance. The proxy will automatically create an appropriate session strategy for concurrent safety by generating a client factory internally.
...
### How Client Factories Work

When you provide a `client` parameter, FastMCPProxy automatically creates a client factory based on the client's state:

```python
# For disconnected clients (recommended for concurrency)
client = Client("backend_server.py")
# Internally creates: lambda: client.new()

# For connected clients (session reuse)
connected_client = Client("backend_server.py")
await connected_client.connect()
# Internally creates: lambda: connected_client
```

You can provide your own client factory for custom behavior:

```python
# Custom factory that reuses the same client instance
def reuse_client():
    return shared_client

proxy = FastMCPProxy(client_factory=reuse_client)

# Custom factory that creates fresh clients with specific configuration
