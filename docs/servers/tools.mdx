...
<ParamField body="serializer" type="ToolResultSerializerType | None = None">
  <VersionBadge version="2.12.5" />
  
  Custom serializer function for tool results. If provided, overrides the server's default `tool_serializer`. The function should accept any value and return a string. See [Custom Serializers](#custom-serializers) for details.
</ParamField>
</Card>
...
You can customize how tool results are serialized to text content by providing a custom serializer function. This is useful when you need specific output formats like YAML, pretty-printed JSON, or integration with high-performance serializers.
...
# Per-tool custom serializer
@mcp.tool(serializer=yaml.dump)
def get_config() -> dict:
    """Returns configuration as YAML."""
    return {"database": "postgres", "port": 5432}

# Server-level default serializer
mcp = FastMCP(tool_serializer=yaml.dump)

@mcp.tool
def get_status() -> dict:
    """Uses server's default YAML serializer."""
    return {"status": "running"}
```

The `serializer` parameter accepts a function that takes any value and returns a string. Per-tool serializers override the server's default `tool_serializer`.

<Tip>
Custom serializers only affect text content blocks. Structured content (when enabled) is always serialized as JSON according to the MCP specification.
</Tip>
