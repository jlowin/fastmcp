...
For additional type annotations not listed here, see the [Argument Types](#argument-types) section below for more detailed information and examples.
...
FastMCP supports a wide variety of argument types to give you flexibility when designing your tools.
...
These types provide clear expectations to the LLM about what values are acceptable and allow FastMCP to validate inputs properly. With the default validation mode (`strict_input_validation=False`), if a client provides a string like `"42"`, it will be automatically coerced to the integer `42` for parameters annotated as `int`.
