...
```python
# Asynchronous tool (preferred for I/O-bound operations)
@mcp.tool
...
# Synchronous tool (suitable for quick, CPU-bound tasks)
@mcp.tool
...
While synchronous tools work seamlessly in FastMCP, they can block the event loop during execution. For CPU-intensive or potentially blocking synchronous operations, consider using `anyio` (which FastMCP already uses internally) to wrap them as async functions:

```python
import anyio
import functools

# For CPU-intensive synchronous functions
def cpu_intensive_task(data: str) -> str:
    # Some heavy computation that could block the event loop
    return processed_data

@mcp.tool
@functools.wraps(cpu_intensive_task)
...
Alternative approaches include using `asyncio.get_event_loop().run_in_executor()` or other threading techniques to manage blocking operations without impacting server responsiveness.
