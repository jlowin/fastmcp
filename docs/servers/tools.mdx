...
You can control how the FastMCP server behaves if you try to register two components with the same name. The `on_duplicate` argument sets that policy once for every component type, so tools, resources, and prompts all follow the same rule.
...
mcp = FastMCP(
    name="StrictServer",
    # Configure behavior for duplicate component names
    on_duplicate="error"
)
